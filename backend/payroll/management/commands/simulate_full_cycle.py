...
def seed_timelogs(employee: Employee, start_date: date, end_date: date) -> None:
    ...
    # Optional holiday on the end date
    holiday_obj = None
    try:
...
class Command(BaseCommand):
    help = "Simulate a FULL month: runs SEMI_1 and SEMI_2, generates PayrollRecords (incl. PH mandatories), and outputs payslip PDFs + monthly summary."

    def add_arguments(self, parser):
        parser.add_argument("--month", required=True, help="YYYY-MM or YYYY-MM-DD (normalized to first day)")
        ...
        parser.add_argument("--no-seed-logs", action="store_true", help="Skip seeding timelogs")
        parser.add_argument("--out-dir", default=".", help="Directory to write payslip PDFs")
    ...
    def handle(self, *args, **opts):
        ...
        if opts.get("employee_id"):
            ...
            employee, _ = Employee.objects.get_or_create(
                ...
                defaults={"branch": branch, "position": position, "hire_date": month, "active": True},
            )

        self.stdout.write(self.style.MIGRATE_HEADING(f"== Full-cycle simulation for {employee} | {month.strftime('%Y-%m')} =="))
        ...
        # Process both semi-monthly cycles
        monthly_totals = {"EARNING": Decimal("0.00"), "DEDUCTION": Decimal("0.00")}
        pdf_paths: list[str] = []

        for cycle in ["SEMI_1", "SEMI_2"]:
            start_date, end_date = get_dynamic_cutoff(month, cycle, business)
            self.stdout.write(self.style.HTTP_INFO(f"[{cycle}] Cutoff {start_date} → {end_date}"))

            # Seed logs optionally
            if not opts["no_seed_logs"]:
            ...
            gross_monthly = compute_regular_monthly_gross(position, base_salary)
            monthly = compute_mandatories_monthly(gross_monthly, policy)
            per_cycle = allocate_to_cycle(monthly, cycle)  # default 50/50 split
            ...
            monthly_totals["EARNING"] += earn
            monthly_totals["DEDUCTION"] += ded

            self.stdout.write(self.style.SUCCESS(f"[{cycle}] Earnings: {earn}  Deductions: {ded}  Net: {earn - ded}  PDF: {pdf_path}"))

        # Combined monthly view (SEMI_1 + SEMI_2)
        net_month = monthly_totals["EARNING"] - monthly_totals["DEDUCTION"]
        self.stdout.write(self.style.MIGRATE_HEADING("== Combined monthly totals (SEMI_1 + SEMI_2) =="))
        self.stdout.write(f"Earnings: {monthly_totals['EARNING']}  Deductions: {monthly_totals['DEDUCTION']}  Net: {net_month}")
        self.stdout.write(self.style.HTTP_INFO("Payslips:"))
        for p in pdf_paths:
